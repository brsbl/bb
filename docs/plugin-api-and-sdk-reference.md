...
Plugin SDK and product versions at this source snapshot: `@bb/plugin-sdk` is `0.4.1`; this document fixes the contract selected for the eventual 1.0 compatibility boundary while keeping it on a pre-1.0 compatibility line.
...
- `engines.bbPluginSdk` describes compatible plugin SDK versions. The current advertised version is `0.4.1`; the scaffold emits `^0.4.1`.
- This release line does not publish 1.0. It remains on the compatible `0.4.x` SDK line; the source snapshot remains `0.x`.
...
- SDK `0.4.1` adds queued-message and side-chat composer scopes plus scope-owned composer/thread-row visual methods. Existing `^0.4.0` plugins remain compatible; plugins that depend on these additions should declare `engines.bbPluginSdk: ^0.4.1`.
- Before 1.0 there is no compatibility promise: backward-compatible additions and clarifications use patch releases within the current `0.x` line, while breaking removals, renames, DTO changes, validation changes, or host-behavior changes require a documented deprecation/removal decision and the appropriate pre-1.0 minor bump. Never silently reinterpret an existing field or routing default.
...
### `sdk.threadSections` — 4 methods

| Method   | Arguments                    | Purpose                                |
| -------- | ---------------------------- | -------------------------------------- |
| `create` | `CreateThreadSectionRequest` | creates a section.                     |
| `delete` | `DeleteThreadSectionRequest` | deletes a section.                     |
| `list`   | none                         | lists sections from sidebar bootstrap. |
| `update` | `UpdateThreadSectionRequest` | updates a section.                     |
...
- **D2 / portable distribution:** package version and `PLUGIN_SDK_VERSION` are both `0.4.1` at this snapshot. Root, `/app`, `/testing`, and `/testing/app` have distributable runtime files and bundled declarations. External scaffold tests run with `skipLibCheck: false` and read representative result fields.
