...
rl.on("line", (line) => {
  ...
  if (message.method === "thread/resume") {
    ...
    const providerThreadId = thread ? thread.providerThreadId : message.params.providerThreadId;
    send({ jsonrpc: "2.0", id: message.id, result: { providerThreadId } });
    return;
  }
...
rl.on("line", (line) => {
  ...
  if (message.method === "thread/resume") {
    const threadId = params.threadId || "unknown";
    const thread = threads.get(threadId);
    const providerThreadId = thread ? thread.providerThreadId : params.providerThreadId;
    send({ jsonrpc: "2.0", id: message.id, result: { providerThreadId } });
    return;
  }
...
function createStandardThreadStartCommand(args: {
  ...
  return {
    ...
    input: args.input.map((item) => ({ ...item, mentions: [] as [] })),
    options: {
      model: "gpt-5",
      serviceTier: "default" as const,
      reasoningLevel: "medium" as const,
      submissionMode: { planMode: "default", goalMode: "none" } as const,
...
describe("host daemon integration", () => {
  it("opens a session, sends heartbeats, reports command results, and does not persist a cursor file", async () => {
    ...
    try {
      ...
      const firstHeartbeatMessage = harness.server.heartbeats.find(
        (h) => h.message.type === "heartbeat",
      );
      expect(firstHeartbeatMessage?.sessionId).toBe("session-1");
      expect(firstHeartbeatMessage?.message).toEqual({ type: "heartbeat" });
      expect(
    ...
    try {
      ...
      harness.server.queueCommand({
        ...createTurnSubmitCommand({
          ...
          projectId: "project-1",
          providerId: "fake",
          providerThreadId: extractProviderThreadId(startResult.result),
          requestId: "creq_23456789ab",
  ...
  it(
    "settles turn-start events before dynamic tool call forwarding",
    async () => {
      const harness = await setupDaemonHarness({
        serverOptions: {
