...
export function filterAndSortNotifications(args: {
  ...
  const filtered = args.items.filter((item) => {
    if (args.resource !== "all" && item.resourceKind !== args.resource)
      return false;
    if (args.activity !== "all" && item.activityKind !== args.activity)
      return false;
...
function UpdatedTime({ value }: { value: string }) {
  ...
  return (
    ...
    >
      <Icon name="Clock" aria-hidden className="size-3.5" />
      {relativeTime(value)}
    </span>
  );
}
...
function SortHeader({
  ...
  return (
    ...
    >
      ...
      {active ? (
        direction === "asc" ? (
          <Icon name="ArrowUp" aria-hidden className="size-3" />
        ) : (
          <Icon name="ArrowDown" aria-hidden className="size-3" />
        )
      ) : (
        <Icon
          name="ArrowUpDown"
      ...
      <div className="mx-auto w-full max-w-6xl">
        <div className="mb-5 flex items-start gap-3">
          <div className="min-w-0 flex-1">
            <p className="text-sm text-muted-foreground">
              Comments and reviews on PRs and issues you authored
        ...
        {error ? (
          <div className="rounded-xl border border-destructive/25 bg-destructive/5 p-4">
            ...
            <button
              type="button"
              className="mt-3 rounded-md border border-border px-2.5 py-1.5 text-sm font-medium text-foreground hover:bg-accent"
              onClick={() => void load(true)}
            >
              Try again
            </button>
          ...
          <div className="rounded-xl border border-dashed border-border p-6 text-center">
            <p className="text-sm font-medium text-foreground">
              {hasFilters
                ...
                : "No comments or reviews to triage"}
            </p>
                ...
                <thead className="border-b border-border bg-muted/35 text-xs">
                  <tr>
                    <th scope="col" className="px-3 py-2.5">
                      <SortHeader
                        label="Item"
                        active={sort === "resource"}
                        direction={direction}
                        onSort={() => setSortKey("resource")}
                      />
                    </th>
                    <th scope="col" className="px-3 py-2.5">
                      <SortHeader
